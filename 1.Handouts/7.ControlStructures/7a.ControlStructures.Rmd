---
title: '7a\. Control Structures and Sampling'
...
These blocks of code serve as algorithms that act on variables in a user-defined way.
...
3. "if" accepts logical values (TRUE or FALSE) often resulting from logical operators  (e.g., <, >=, !=. see `?Logic`); can accept numbers (0 is FALSE, any other number is TRUE) but not strings, which are arrays of characters (e.g., "ATCCCGCT" or "purple martin")  
...
The first part, sometimes referred to as the **header**, specifies the iteration and has a loop counter, often specified by a letter (e.g., *i*, *j*, *k*), which is referred to as the **index**
...
In the above for-loop structure, *i* is the index variable that will hold each integer from 1 to 10 during each successive pass through the loop.  
...
Note that a for-loop can iterate over any type of vector: logical, numerical, string. In this example assign the iteration counter variable to hold the value of each item in our vector. You can also iterate over vectors using indices (R counts from 1 and not from 0 like some other languages).     
...
You can find them listed in the R Documentation by typing `?Distributions` into the console.  
...
Let's look at the uniform distribution where all values have an equal probability. 
In R, the uniform distribution is called using *unif* using one of the following four functions:     
...
If you enter same seed, you will get the same sequence of numbers.   
...
Here is how it looks in a user-defined function:  
...
plot.new()
par(mfrow = c(1, 2))
plot(RACresults, las = 1, cex.lab = 1.4, cex.axis = 1.25, xlim = c(1, 20), 
     main = "Source community")
plot(RACsample, las = 1, cex.lab = 1.4, cex.axis = 1.25, xlim = c(1, 20), 
     main = "Sample")
```
